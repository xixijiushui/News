...
extension UILabel {
    ...
    /// 设置问答的内容
    func setSeparatedLinesForm(_ attributedString: NSMutableAttributedString, hasImage: Bool) {
        // 通过coretext 创建字体
...
extension UICollectionView {
    // 注册 cell 的方法
    func zw_registerCell<T: UICollectionViewCell>(cell: T.Type) where T: RegisterCellOrNib {
        if let nib = T.nib {
    ...
    // 从缓存池出队已经存在的cell
    func zw_dequeueReusableCell<T: UICollectionViewCell>(indexPath: IndexPath) -> T where T: RegisterCellOrNib {
...
extension UITableView {
    // 注册 cell 的方法
    func zw_registerCell<T: UITableViewCell>(cell: T.Type) where T: RegisterCellOrNib {
        if let nib = T.nib {
    ...
    // 从缓存池出队已经存在的cell
    func zw_dequeueReusableCell<T: UITableViewCell>(indexPath: IndexPath) -> T where T: RegisterCellOrNib {
