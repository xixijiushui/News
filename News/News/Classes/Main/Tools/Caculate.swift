...
protocol Calculatable {
    ...
    // MARK: 计算详情里的高度
    static func detailCollectionViewHeight(_ thumbImageList: [ThumbImageList]) -> CGFloat
}
...
extension Calculatable {
    ...
    /// 计算文本的宽度
    static func textWidth(text: String, fontSize: CGFloat, height: CGFloat) -> CGFloat {
        return text.boundingRect(with: CGSize(width: CGFloat(MAXFLOAT), height: height), options: .usesLineFragmentOrigin, attributes: [.font : UIFont.systemFont(ofSize: fontSize)], context: nil).size.width + 5.0
    }
    ...
    static func detailCollectionViewHeight(_ thumbImageList: [ThumbImageList]) -> CGFloat {
        switch thumbImageList.count {
        case 1:
            let thumbImge = thumbImageList.first!
            return (screenWidth - 30) * thumbImge.height / thumbImge.width
        case 2:
