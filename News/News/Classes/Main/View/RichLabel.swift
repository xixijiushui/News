...
class RichLabel: UILabel {
    ...
    /// 并将字符编码映射到对应的字形上, 然后将字形a排版到 NSTextContainer 定义的区域中
...
extension RichLabel {
    ...
    private func range(of point: CGPoint) -> NSRange {
        // 判断 textStorage 长度是不是为0
        if textStorage.length == 0 {
            tapRichTextType = .none
            return NSRange()
        }
        // 在 textStorage 中的索引
